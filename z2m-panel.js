...
/* Botón menú — solo visible en móvil/tablet (sin sidebar de HA) */
.back-btn{
  width:34px;height:34px;border-radius:50%;border:none;
  background:transparent;color:var(--tint);
  display:flex;align-items:center;justify-content:center;
  cursor:pointer;font-size:24px;transition:all .15s;
  flex-shrink:0;padding:0;line-height:1;
}
.back-btn:hover{background:var(--tint-bg);}
.back-btn:active{transform:scale(.88);}

/* HA muestra sidebar a partir de 870px — ocultar botón menú en esos casos */
...
#err-401.on{display:flex;}
.err-401-box{
  background:#1a0000;border:2px solid var(--red);border-radius:16px;
  padding:28px 28px 22px;max-width:680px;width:100%;max-height:85vh;
  overflow-y:auto;box-shadow:0 0 60px rgba(255,59,48,.35);
}
.err-401-head{display:flex;align-items:center;gap:12px;margin-bottom:18px;}
.err-401-emoji{font-size:2.4rem;line-height:1;}
.err-401-title{font-size:1.15rem;font-weight:800;color:#ff3b30;letter-spacing:-.02em;}
.err-401-title small{display:block;font-size:.72rem;font-weight:500;color:#ff6b63;margin-top:2px;letter-spacing:0;}
.err-401-table{width:100%;border-collapse:collapse;font-size:.78rem;margin-bottom:14px;}
.err-401-table tr td:first-child{
  color:#ff8a80;font-weight:700;padding:5px 12px 5px 0;
  white-space:nowrap;vertical-align:top;width:140px;
}
.err-401-table tr td:last-child{color:#ffd0ce;word-break:break-all;padding:5px 0;}
.err-401-pre{
  background:#0d0000;border:1px solid #5c0000;border-radius:8px;
  padding:12px;font-size:.72rem;color:#ffa9a3;font-family:monospace;
...
.err-401-close:hover{opacity:.85;}
...
<div id="root">
  <div id="navbar">
    <div class="nav-left">
      <button class="back-btn" id="btn-back" title="Menú">☰</button>
  ...
  <!-- OVERLAY 401 -->
  <div id="err-401" role="alertdialog" aria-modal="true">
    <div class="err-401-box">
      <div class="err-401-head">
        <span class="err-401-emoji">🚨</span>
        <div class="err-401-title">Error 401 — No autorizado<small>El token de HA ha sido rechazado. Datos de debug:</small></div>
      </div>
      <table class="err-401-table" id="err-401-table"></table>
      <div class="err-401-pre" id="err-401-body">—</div>
      <button class="err-401-close" id="err-401-close">Cerrar</button>
    </div>
  </div>
  <main>
    <div id="new-section">
...
class Z2MPanel extends HTMLElement {
  ...
  _wsCall(type, extra = {}) {
    // Reutiliza la conexión WS autenticada que hass ya mantiene abierta.
    // Evita abrir conexiones nuevas (y re-autenticar) en cada llamada,
    // lo que generaba intentos de login fallidos y posibles IP bans.
    return this._hass.connection.sendMessagePromise({ type, ...extra });
  }

  async _mqttPub(topic, payload) {
    const r = await this._haFetch(`${this._haUrl}/api/services/mqtt/publish`, {
  ...
  _show401Debug(ctx) {
    const overlay = this._$('err-401');
    const table   = this._$('err-401-table');
    const body    = this._$('err-401-body');
    if (!overlay) return;
    const token = this._token || '';
    const rows = [
      ['Contexto',    ctx.context || '—'],
      ['URL',         ctx.url     || '—'],
      ['Método',      ctx.method  || 'GET'],
      ['HTTP Status', ctx.status  || '401'],
      ['Timestamp',   new Date().toISOString()],
      ['HA URL',      this._haUrl || '—'],
      ['Token prefix', token ? token.slice(0, 20) + '…' : '(vacío)'],
      ['Token length', token.length || 0],
    ];
    table.innerHTML = rows.map(([k,v]) =>
      `<tr><td>${k}</td><td>${String(v).replace(/</g,'&lt;')}</td></tr>`
    ).join('');
    body.textContent = ctx.body || '(sin cuerpo)';
    overlay.classList.add('on');
    const btn = this._$('err-401-close');
    if (btn) btn.onclick = () => overlay.classList.remove('on');
  }

  async _haFetch(url, opts = {}, context = '') {
    let r;
    ...
    if (r.status === 401) {
      ...
      this._show401Debug({
        context, url, method: opts.method || 'GET', status: 401, body
      });
  ...
  async _load(showSpinner = false) {
    ...
    try {
      const [statesR, wsDevs, wsEnts, z2mData] = await Promise.all([
        this._haFetch(`${this._haUrl}/api/states`, { headers: this._hdrs }, 'load:states').then(r => r.json()),
        this._wsCall('config/device_registry/list'),
        this._wsCall('config/entity_registry/list'),
        this._getZ2MDevices(),
      ]);

      if (!wsDevs) throw new Error('No se pudo obtener el registro');

      const allEnts = wsEnts || [];
      ...
      const z2m = wsDevs.filter(d => d.via_device_id === BRIDGE_ID);
      ...
      this._ieeeToZ2mName = Object.fromEntries(Object.entries(this._friendlyToIeee).map(([k, v]) => [v, k]));
      this._devsByIeee = {};
  ...
  _patchCard(d, card) {
    ...
    card.onclick = () => this._openEntPopup(d);
    const renBtn = card.querySelector('.act-btn:not(.del)');
    if (renBtn) renBtn.onclick = (e) => { e.stopPropagation(); this._openRename(d.name); };
    const delBtn = card.querySelector('.act-btn.del');
    if (delBtn) delBtn.onclick = (e) => { e.stopPropagation(); this._openDelete(d); };
  ...
  _makeCard(d) {
    ...
    card.querySelector('.act-btn').onclick = (e) => { e.stopPropagation(); this._openRename(d.name); };
    card.querySelector('.act-btn.del').onclick = (e) => { e.stopPropagation(); this._openDelete(d); };
    card.onclick = () => this._openEntPopup(d);
    return card;
  }
  ...
  async _doRename() {
    ...
    // Guardar entidades actuales antes de renombrar (para actualizarlas en HA)
    const dev = this._devices.find(d => d.name === this._renameFrom);
    const devEnts = dev ? (dev.ents || []).map(e => e.entity_id) : [];

    this._closeModal('m-rename');
    try {
      await this._mqttPub('zigbee2mqtt/bridge/request/device/rename', { from: this._renameFrom, to: n });

      // Renombrar entidades en HA: domain.old_slug → domain.new_slug
      if (devEnts.length) {
        const toSlug = s => s.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
        const oldSlug = toSlug(this._renameFrom);
        const newSlug = toSlug(n);
        for (const entityId of devEnts) {
          const dot = entityId.indexOf('.');
          if (dot < 0) continue;
          const domain = entityId.slice(0, dot);
          const suffix = entityId.slice(dot + 1);
          if (!suffix.startsWith(oldSlug)) continue;
          const newEntityId = `${domain}.${newSlug}${suffix.slice(oldSlug.length)}`;
          if (newEntityId === entityId) continue;
          try {
            await this._wsCall('config/entity_registry/update', { entity_id: entityId, new_entity_id: newEntityId });
          } catch(e) {} // Si ya existe o conflicto, skip silencioso
        }
      }
  ...
  _startEventListener() {
    ...
    ws.onmessage = ev => {
      ...
      if (m.type === 'auth_required') {
        ...
        authFailed = true; ws.close();
        this._show401Debug({ context: 'eventListener:auth_invalid', url: wsUrl, method: 'WS', status: 401, body: 'auth_invalid en WS de eventos' });
        return;
      } else if (m.type === 'auth_ok') {
    ...
    ws.onerror = () => {};
    ws.onclose = () => {
      if (this._eventWs === ws && !authFailed) {
  ...
  async _openEntPopup(d) {
    ...
    let stateMap = {};
    try {
      const resp = await this._haFetch(`${this._haUrl}/api/states`, { headers: this._hdrs }, 'popup:states');
    ...
    if (d.type === 'switch' || d.type === 'light') {
      ...
      cvtBtn.onclick = async () => {
        ...
        try {
          if (d.type === 'switch') {
            // switch → luz: crear ayudante switch_as_x vía /api/config/config_entries/flow
            const switchEnt = (d.ents || []).find(e => !e.disabled_by && e.entity_id.startsWith('switch.'));
            if (!switchEnt) throw new Error('No hay entidad switch activa');
            const eid = switchEnt.entity_id;

            // Paso 1: iniciar flujo
            const r1 = await fetch(`${this._haUrl}/api/config/config_entries/flow`, {
              method: 'POST', headers: this._hdrs,
              body: JSON.stringify({ handler: 'switch_as_x' })
            });
            if (!r1.ok) throw new Error(`init flow ${r1.status}: ${(await r1.text()).slice(0,100)}`);
            const d1 = await r1.json();

            // Paso 2: enviar formulario
            const r2 = await fetch(`${this._haUrl}/api/config/config_entries/flow/${d1.flow_id}`, {
              method: 'POST', headers: this._hdrs,
              body: JSON.stringify({ entity_id: eid, target_domain: 'light' })
            });
            if (!r2.ok) throw new Error(`submit flow ${r2.status}: ${(await r2.text()).slice(0,100)}`);
            const d2 = await r2.json();
            if (d2.type !== 'create_entry') throw new Error(`Flow inesperado: ${d2.type}`);

            this._toast('ok', `✅ Convertido a ${targetLabel}`);
            popup.style.display = 'none';
            const ieee1 = d.ieee;
            setTimeout(async () => { await this._load(); const upd = this._devsByIeee[ieee1]; if (upd) this._openEntPopup(upd); }, 5000);
          } else {
            // luz → switch: eliminar switch_as_x o revertir MQTT discovery
            const switchAsXEnt = (d.ents || []).find(e => e.platform === 'switch_as_x');
            const hiddenSwitch = (d.ents || []).find(e => e.entity_id.startsWith('switch.') && e.hidden_by === 'user');

            if (switchAsXEnt?.config_entry_id) {
              // Eliminar config entry switch_as_x — mismo path base que crear (/api/config/...)
              const rd = await fetch(`${this._haUrl}/api/config/config_entries/entry/${switchAsXEnt.config_entry_id}`, {
                method: 'DELETE', headers: this._hdrs
              });
              if (!rd.ok) throw new Error(`delete ${rd.status}: ${(await rd.text()).slice(0,100)}`);
            } else if (hiddenSwitch) {
              // Fue convertido via MQTT discovery (versiones antiguas) — revertir
              const ieeeSafe = d.ieee.replace(/[^a-z0-9]/gi, '_');
              const ru = await this._haFetch(`${this._haUrl}/api/services/mqtt/publish`, {
                method: 'POST', headers: this._hdrs,
                body: JSON.stringify({ topic: `homeassistant/light/z2mp_${ieeeSafe}/config`, payload: '', retain: true })
              });
              if (!ru.ok) throw new Error(`mqtt remove ${ru.status}`);
              try { await this._wsCall('config/entity_registry/update', { entity_id: hiddenSwitch.entity_id, hidden_by: null }); } catch {}
            } else {
              throw new Error('No se encontró ayudante switch_as_x para revertir');
            }
            this._toast('ok', `✅ Convertido a ${targetLabel}`);
            popup.style.display = 'none';
            const ieee2 = d.ieee;
            setTimeout(async () => { await this._load(); const upd = this._devsByIeee[ieee2]; if (upd) this._openEntPopup(upd); }, 2500);
          }
        } catch(e) {
          this._toast('err', 'Error: ' + (e.message || String(e)));
  ...
  _startMqttStateListener() {
    ...
    ws.onmessage = ev => {
      ...
      if (m.type === 'auth_required') {
        ...
        authFailed = true; ws.close();
        this._show401Debug({ context: 'mqttStateListener:auth_invalid', url: wsUrl, method: 'WS', status: 401, body: 'auth_invalid en WS de estado MQTT' });
        return;
      } else if (m.type === 'auth_ok') {
    ...
    ws.onerror = () => {};
    ws.onclose = () => {
      if (this._mqttStateWs === ws && !authFailed) {
